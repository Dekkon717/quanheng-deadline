  ...
  return (
    <div className="result-wrap v11-result">
      ...
      <section className="result-section">
        ...
        {professional ? <div className="rule-trace"><p>规则追溯</p><div><code>{result.rule.id}</code><span>{result.rule.provision} · 核验于 {VERIFIED_AT}</span></div></div> : null}
      </section>
  ...
  return (
    <main className="app-shell v11-shell">
      <header className="topbar"><div className="topbar-inner">
        <button type="button" className="brand" onClick={() => { setView('calculator'); resetFields(); }}><span className="brand-mark">时</span><span><strong>权衡 · 法律期限助手</strong><small>{offline ? '规则与计算均在本机运行 · 离线版' : '每个日期，都能回到现行法条'}</small></span></button>
      ...
      <section className="catalog-hero"><div><p className="eyebrow">中国大陆 · 现行国家层面规则 · 核验至 {VERIFIED_AT}</p><h1>先识别是哪一种期限，<br />再开始算日期。</h1><p>覆盖民事诉讼时效与权利期间、刑事追诉期限、行政起诉期限和行政复议申请期限。复杂事实不猜，直接转人工判断。</p></div><div className="hero-stats"><div><strong>{DEADLINE_RULES.length}</strong><span>项规则</span></div><div><strong>{automaticCount}</strong><span>项可计算</span></div><div><strong>{DEADLINE_SOURCES.length}</strong><span>份官方法源</span></div></div></section>
      ...
      {view === 'calculator' ? (
        ...
        <section className="library-shell">
          <div className="library-tools"><div><p className="eyebrow">中国大陆法律期限规则库</p><h2>检索法条、期限和案件类型</h2></div><input aria-label="搜索规则" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="例如：追诉期限、行政复议、保证期间、上诉…" /></div>
